...
class PedidoBebidaController extends Controller
{
    ...
    public function goComandaBebida()
    {
        if (!$this->validarSesion("bebida_cmds")) {
            return redirect('/');
        }
        ...
        return view('cocina.bebidas.comandas', compact('data'));
    }
    ...
    public static function getOrdenesEnPreparacionBebida($restaurante)
    {
        if ($restaurante < 1 || $restaurante == null) {
            return [];
        }
        ...
        $ordenesConDetalle = DB::table('detalle_orden')
            ...
            ->whereIn('orden.estado', array('EP', 'EPF'))
            ->where('orden.restaurante', '=', $restaurante)
            ->where('orden.bebida_terminado', '=', 'N')
            ->where('detalle_orden.tipo_comanda', '=', 'BE')
            ->orderBy('detalle_orden.fecha_creacion', 'ASC')->distinct()->get();
    ...
    public function terminarPreparacionOrdenBebida(Request $request)
    {
        if (!$this->validarSesion("bebida_cmds")) {
        ...
        try {
            DB::beginTransaction();
            $bebidasTerminadas = $this->bebidasTerminadas($id_orden, $fecha_detalle);
            
            $detalles = DB::table('detalle_orden')->select('detalle_orden.*')
            ->where('orden', '=', $id_orden)
            ->where('tipo_comanda', '=', 'BE')
            ...
            if ($bebidasTerminadas) {
                if ($orden->cocina_terminado == 'N') {
                    if ($orden->estado == 'EPF') {
                        $estado = 'EPF';
                    } else {
                        $estado = 'EP';
                ...
                DB::table('orden')
                    ->where('id', '=', $id_orden)
                    ->update([
                        'estado' => $estado, 'fecha_preparado' => date("Y-m-d H:i:s"), 'bebida_terminado' => 'S'
                    ]);
            }
    ...
    public function bebidasTerminadas($orden, $fecha_detalle)
    {
    ...
    public function recargarOrdenesEsperaBebida(Request $request)
    {
        if (!$this->validarSesion("bebida_cmds")) {
        ...
        return view('cocina.bebidas.layout.contenedor_comandas', compact('data'));
    }
}
