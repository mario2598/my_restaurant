...
function generarHTMLProducto(nombre, codigo, precio) {
    ...
    if (tipoOrden != 'LL') {
        texto += ` <button type="button" class="btn btn-info px-2" onclick="seleccionarProducto('S','${codigo}')"><i class="fas fa-utensils" aria-hidden="true"></i></button>`;
    }
    texto += `</td> </tr>`;
    return texto;
}
...
function seleccionarProducto(impuestoServicio, codigo, todos = false) {
    ...
    if (producto !== undefined) {
        if (validarCantidadProducto(producto)) {
            let indice = buscarDetallePrevio(impuestoServicio, codigo,0);
            if (indice >= 0) {
...
function buscarDetallePrevio(impuestoServicio, codigoProducto,fechaCreacion) {
    let indice = -1;
    let contador = 0;
    let aumentar = true;
    detalles.forEach(detalle => {
        if (detalle.producto.codigo == codigoProducto && detalle.impuestoServicio == impuestoServicio && detalle.fechaCreacion == fechaCreacion) {
            indice = contador;
            aumentar = false;
        }
        if (aumentar) {
...
function actualizarDetalleOrden(indice, aumenta = true) {
    ...
    if (aumenta) {
        detalle.cantidad = parseInt(detalle.cantidad) + 1;
...
function actualizarOrden() {
    ...
    let total = 0;
    detalles.forEach(detalle => {
        cards += generarHTMLProductoOrden(contador, detalle.producto.nombre, parseFloat(detalle.producto.precio).toFixed(2), detalle.cantidad, parseFloat(detalle.total).toFixed(2), detalle.producto.codigo, detalle.impuestoServicio, detalle.cantidadPreparada);
        let totalAux = detalle.cantidad * parseFloat(detalle.precio_unidad);
        total = total + totalAux;
        if (detalle.impuestoServicio == 'S') {
            let impuestoMesa =  totalAux -(totalAux / 1.10); 
 ...
 * Genera el elemento HTML correspondiente a la categoría
 */
function generarHTMLProductoOrden(indice, detalle, precio, cantidad, total, codigo, impuestoServicio = "N", cantidadPreparada) {
    ...
    if (cantidadPreparada < cantidad) {
        texto += `<button type="button" class="btn btn-danger px-2" onclick="actualizarDetalleOrden(${indice},false)"><i
        class="fas fa-trash" aria-hidden="true"></i></button>`;
    }
    texto += `<button type="button" class="btn btn-warning px-2" title="Agregar observación a la orden" onclick="agregarDescripcionDetalle(${indice})"><i
                            class="fas fa-clipboard" aria-hidden="true"></i></button></td></tr>`;
    return texto;
}
...
function guardarFactura(seFactura) {
    if (!facturando) {
        facturando = true;
        iziToast.success({
            title: 'Actualizar Orden',
            message: 'Actualizando orden...',
            position: 'topRight'
        });
        if (validarFormularioOrden()) {
            try {
                ...
                $.ajax({
                    url: `${base_path}/cocina/facturar/ordenes/actualizarOrden`,
                    type: 'post',
                    dataType: "json",
                    data: {
                        _token: CSRF_TOKEN,
                    ...
                    if (!res['estado']) {
                        swal('Actualizar Orden', res['mensaje'], 'error');
                    } else {
                        let datos = res["datos"];

                        if (seFactura) {
                            swal('Orden Actualizada!', 'Redirigiendo al pago.', 'success');
        ...
        iziToast.error({
            title: 'Actualizar Orden',
            message: 'Ya se esta procesando la orden..',
            position: 'topRight'
        });
    }
}
...
function goDividirFactura(id) {
    $("#ipt_id_orden_dividir").val(id);
    $("#frm-dividirFactura").submit();
}
